...
def compute_tag_correctness(final_results):
...
def compute_semantic_repetition(final_results):
    texts = [_normalize_for_similarity(x["text"]) for x in final_results]
    n = len(texts)
...
def compute_all_metrics(final_results):
    return {
        "tag_correctness": compute_tag_correctness(final_results),
        "semantic_repetition": compute_semantic_repetition(final_results),
    }


def main():
    with open("final_results.json", "r", encoding="utf-8") as f:
        final_results = json.load(f)["final_results"]
    report = compute_all_metrics(final_results)
    print(report)
